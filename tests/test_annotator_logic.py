...
def test_get_homography_and_project_full_template():
    state = AnnotationState()
    # Place the 4 outer corners matching the court's aspect ratio (roughly).
    state.set_keypoint(0, 0.0, 0.0)
    state.set_keypoint(1, 1.0, 0.0)
    state.set_keypoint(2, 1.0, 1.0)
...
def test_is_corner_quad_valid_false_for_self_intersecting():
    state = AnnotationState()
    # Bowtie ordering: swap K2 and K3 positions relative to a proper rectangle.
    state.set_keypoint(0, 0.1, 0.1)
    state.set_keypoint(1, 0.9, 0.9)
    state.set_keypoint(2, 0.9, 0.1)
    state.set_keypoint(3, 0.1, 0.9)
    assert state.is_corner_quad_valid() is False
